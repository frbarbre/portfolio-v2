    ...
    >
      <article className="flex-1">
        ...
        <p className="text-[14px] font-medium tracking-[0.84px] md:text-[18px] md:tracking-[1.08px]">
          {language === "en"
            ? paragraphSplitEN.map((paragraph) => (
            ...
            : paragraphSplitDA.map((paragraph) => (
                <Fragment key={paragraph}>
                  {paragraph}
                  <br />
                  <br />
      ...
      <Image
        ...
        width={1200}
        height={1200}
        className={`lg:flex-1 rounded-[5px] md:rounded-[10px] w-full min-h-[366px] object-about object-cover ${theme === "light" ? "border-black/20" : "border-white/20"} border-[2px]`}
      />
    </section>
  );
}
